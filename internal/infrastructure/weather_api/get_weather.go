...
func (w *WeatherApiForecast) GetWeatherForecast(input *dto.GetWeatherRequest) (*dto.GetWeatherResponse, error) {
	log.Println("FFFF")
...
func (w *WeatherApiArchive) GetWeatherArchive(input *dto.GetWeatherRequest) (*dto.GetWeatherResponse, error) {
	log.Println("AAAA")
	latitude := strconv.FormatFloat(input.Latitude, 'g', -1, 32)
	longitude := strconv.FormatFloat(input.Longitude, 'g', -1, 32)
	...
	req, err := http.NewRequest(http.MethodGet, w.host+"/archive", http.NoBody)
	if err != nil {
		return nil, fmt.Errorf("http.NewRequest: %w", err)
	}
